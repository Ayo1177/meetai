...
export async function POST(req: NextRequest) {
    ...
    const [existingMeeting] = await db
        ...
        let instructions = existingAgent.instructions || "You are an AI assistant.";
      ...
      ${existingMeeting.summaryUrl || "No summary available"}
      ...
      `;
        ...
        if(!GPTResponseText) {
            return NextResponse.json(
                { error: "Missing GPT response text" },
                { status: 400 }
            );
        }
        
        const avatarUrl = generatedAvatrUri({
