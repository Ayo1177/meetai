...
export const meetingsRouter = createTRPCRouter({
   generateChatToken: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.auth.data?.user.id;
    if (!userId) {
   ...
   getTranscript: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
        const [existingMeeting] = await db
            ...
            .from(meetings)
            .where(
                and(eq(meetings.id, input.id), eq(meetings.userId, ctx.auth.data!.session.userId))
            );
        ...
        const userSpeakers = await db
                ...
                .then((users) => 
                    users.map((user) => ({
                        ...
                        image:
                            user.image ??
                            generatedAvatrUri({ seed: user.name, variant: "initials" })
                    }))
                );
        ...
        const agentSpeakers = await db
                ...
                .then((agents) => 
                    agents.map((agent) => ({
                        ...agent,
                        image: generatedAvatrUri({ seed: agent.name, variant: "botttsNeutral" })
                    }))
                );
        ...
        const transcriptWithSpeakers = transcript.map((item) => {
                ...
                if (!speaker) {
                    return {
                        ...
                        user: { 
                            name: "Unknown",
                            image: generatedAvatrUri({ seed: "Unknown", variant: "initials" })
                        }
                    };
                }
                return {
